...
class Database:
    ...
    def get_funnyUsers_length(self):
        """
    ...
    def get_seriousUsers_length(self):
        ...
        returns:
            - A list of all user information in the funnyUsers database.
        """
        self.cursor.execute("SELECT COUNT(*) FROM seriousUsers")
        count = self.cursor.fetchone()[0]
        return count
    
    def get_and_remove_first_funnyUser(self):
        """
        Gets the first funnyUser from the funnyUsers table and removes that row.

        args:
            - None

        returns:
            - The username and sessionID of the first funnyUser as a tuple, or None if no users are available.
        """
        self.cursor.execute("SELECT * FROM funnyUsers LIMIT 1")
        first_user = self.cursor.fetchone()

        username, sessionID = first_user
        self.cursor.execute("DELETE FROM funnyUsers WHERE username = ?", (username,))
        return username, sessionID

    
    def get_and_remove_first_seriousUser(self):
        """
        Gets the first funnyUser from the funnyUsers table and removes that row.
        ...
        returns:
            - The username and sessionID of the first funnyUser as a tuple, or None if no users are available.
        """
        self.cursor.execute("SELECT * FROM seriousUsers LIMIT 1")
        first_user = self.cursor.fetchone()

        username, sessionID = first_user
        self.cursor.execute("DELETE FROM seriousUsers WHERE username = ?", (username,))
        return username, sessionID
    
    def remove_user_from_queues(self, username):
        """
        Removes the user from either the funnyQueue or seriousQueue.
        ...
        try: 
            ...
            print(f"Error removing user '{username}' from queue:", str(e))
    ...
    def clear_queues(self):
        ...
        returns:
            - None
        """
        self.cursor.execute("DELETE FROM funnyUsers")
        self.cursor.execute("DELETE FROM seriousUsers")
        self.connection.commit()